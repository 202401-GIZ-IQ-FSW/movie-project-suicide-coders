...
const MoivesQuery = ({title, data, pageNumber}) => {
  ...
  const handleClick = ({target}) => {
    router.push(`/movies?movies=${title}&page=${target.value}${genreParams ? "&genre="+genreParams: ""}`)
  }
  ...
  return (
    <div className="py-2 lg:px-10 px-5">
      <div className="flex justify-between items-center">
        ...
        <button onClick={()=>{
            pageNumber > 1 ? router.push(`/movies?movies=${title}&page=${pageNumber - 1}${genreParams ? "&genre="+genreParams: ""}`): false;
          ...
          <button onClick={()=>{
            router.push(`/movies?movies=${title}&page=${1}${genreParams ? "&genre="+genreParams: ""}`)
          ...
          <button onClick={()=>{
            pageNumber < 500 ? router.push(`/movies?movies=${title}&page=${pageNumber - 1 + 2}${genreParams ? "&genre="+genreParams: ""}`): false;
          }} className="join-item btn md:text-[16px] md:mx-2 text-red-600"><FontAwesomeIcon icon={faForward}/></button>
        </div>}

      </div>
      <div className="flex flex-wrap gap-5 items-center py-5 justify-center">
